#!/usr/bin/env ts-node
// Seed Delivrd War Room with realistic test data
...
// ============================================
async function main() {
  console.log('🌱 Starting Delivrd War Room data seed...\n');

  try {
    // Get auth users for assigned_to references
    console.log('📝 Fetching auth users...');
    const { data: profiles } = await supabase.from('profiles').select('*');
    const tomiProfile = profiles?.find(p => p.id); // Use first available profile
    
    if (!tomiProfile) {
      console.log('⚠️  No profiles found. Proceeding without assigned_to references.');
    }
    ...
    for (const task of FOLLOW_UPS) {
      ...
      if (contact) {
        ...
        followUpsToInsert.push({
          contact_id: contact.id,
          assigned_to: tomiProfile?.id || contact.assigned_to,
          created_by: tomiProfile?.id,
          title: task.title,
